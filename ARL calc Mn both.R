...
ARL_Main_stat_M_n_in_control_both <- function(CL, CL_gray){    
  ...
  load(file = "trace_cov_for_20_300.Rdata")
  
  load(file = "sq_trace_cov_for_20_gray.Rdata")
  load(file = "trace_cov_for_20_gray.Rdata")
  ...
  counts_sim <- foreach(n=1:1000, .combine="rbind", .inorder = FALSE) %dopar% {
    ...
    for(j in 21:(steps+20)){
      ...
      if(color){
        ...
        for (tau in 1:(j-1)) {
          ...
          some_help[tau] <- (j-tau+1)*t((rowMeans(xr[,tau:j,drop=FALSE])-Mean_im_RGB))%*%(rowMeans(xr[,tau:j,drop=FALSE])-Mean_im_RGB)
        }
        Result <- (max(some_help)- trace_cov_for_20 )/sq_trace_cov_for_20
      ...
      if(gray){
        ...
        for (tau in 1:(j-1)) {
          ...
          some_help[tau] <- (j-tau+1)*t((rowMeans(xr_gray[,tau:j,drop=FALSE])-Mean_im_gray))%*%(rowMeans(xr_gray[,tau:j,drop=FALSE])-Mean_im_gray)
          
        }
        Result <- (max(some_help)- trace_cov_for_20_gray )/sq_trace_cov_for_20_gray
...
sim_for_ARL_in_control_M_both<-ARL_Main_stat_M_n_in_control_both(3.17, 3.185)
sim_for_ARL_in_control_M_both
